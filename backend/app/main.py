...
app = FastAPI(
    ...
    description="NL → Workflow DSL → semantic interpreter (OpenRouter via LiteLLM)",
    version="0.4.0",
)
...
def _raise_if_llm_unreachable(exc: BaseException) -> None:
    ...
    hint = (
        ...
        "or VLLM_BASE_URL for a local OpenAI-compatible server."
    )
    raise HTTPException(
        status_code=503,
...
async def list_models():
    return {
        ...
        "note": "Set DEFAULT_MODEL and OPENROUTER_API_KEY. Optional VLLM_BASE_URL for local servers.",
    }
...
async def workflow_nl_to_dsl(request: NLToDSLRequest):
    ...
    if not resolve_api_key() and not vllm_base_url():
        raise HTTPException(
            status_code=503,
            detail="Set OPENROUTER_API_KEY (or VLLM_BASE_URL for a local server).",
        )

    model = request.model or DEFAULT_MODEL
    try:
        llm = make_litellm_caller(model)
